...
M = 30; % m
Q = 0.03; %0.02 ; %   % injected volume flow rate m^3/second
...
%% Temperature calc
if PlotT
    T_tphi = T_Schulz( Xmesh, Ymesh, t, v_u, K, n, Cw, Cs, l_s, T0, Ti, alpha_deg, M, Q, a, delta_phi, N );
end
%% calculate hydraulic potential phi 
I_phi = schulz_Iphi( N, x_0, y_0, v_u, K, alpha_deg, M, Q, a, delta_phi );
...
if PlotT
contour( Xmesh, Ymesh, Kelvin2DegC(T_tphi), [26, 28, 33, 40], 'ShowText','on', 'LineWidth', 2)
end
% Streamline from initial point
for i = 1 : numel(x_0)
    [ x_list, y_list ] = schulz_xy_streamline( N, x_0(i), y_0(i), v_u, K, alpha_deg, M, Q, a, delta_phi );
