function yi = interp_special(varargin)
...
% modification of original matlab code!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
% forcing the x vector to be monotonic, otherwise there is a problem of equal subsequent values
lx=length(x);
for ii=1:lx-3
   if x(ii)==x(ii+1)
      if x(ii)~=x(ii+2)
         x(ii+1)=x(ii+1)+[x(ii+2)-x(ii+1)]/2;
      else
         x(ii+1)=x(ii+1)+[x(ii+3)-x(ii+1)]/3;
         x(ii+2)=x(ii+2)+[x(ii+3)-x(ii+1)]/2;
      end
   end 
   if x(ii)>x(ii+1)
      temp=x(ii);
...
if x(lx-2)==x(lx-1)
   x(lx-1)=x(lx-1)+0.01*(x(lx)-x(lx-2));
end
if x(lx-1)>x(lx)
      temp=x(lx);
      x(lx)=x(lx-1);
